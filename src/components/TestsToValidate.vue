<template>
  <v-card>
    ...
    <v-expand-transition>
      <div v-show="show" float="right">
        <v-card-text>
          <v-row>
            <v-col cols="6"> </v-col>
            <v-col cols="2">
              <v-checkbox
                v-model="showUnverifiedTests"
                label="Unverified"
                class="mb-2 ml-2"
                hide-details
              ></v-checkbox>
    ...
    >
      ...
      <template v-slot:item.actions="{ item }">
        <span style="white-space: nowrap;">
          <v-tooltip top v-if="!item.encounter && item.status !== 'deleted'">
            <template v-slot:activator="{ on }">
              <v-icon
                v-on="on"
                color="orange"
                class="mr-2"
                @click="openTestSOAPDialog(item)"
              >
                mdi-form-select
              </v-icon>
            </template>
          ...
          <v-tooltip top v-if="item.status !== 'deleted'">
            <template v-slot:activator="{ on }">
              <v-icon
                v-on="on"
                color="orange"
            ...
            <v-icon
              ...
              :disabled="
                !(
                  ...
                  (item.type === 'covid19' && !item.covid ? false : true)
                )
              "
              :color="
                item.encounter &&
                ...
                (item.type === 'covid19' && !item.covid ? false : true)
                  ...
                  : 'grey'
              "
              @click="setTestStatus(item, 'verified')"
            >
...
export default {
    ...
    headers() {
      return [
        ...
        { text: 'Results', value: 'results', width: 200 },
        {
          ...
          width: 130,
          filter: (value) => {
            if (value == 'deleted') return this.showDeletedTests
            if (value == 'unverified') return this.showUnverifiedTests
            return false
          },
        },
      ]
  ...
  methods: {
    fetchItems() {
      API.get('tests?status=deleted,unverified&encounter=true&response=true', {
